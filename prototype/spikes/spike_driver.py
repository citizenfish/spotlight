...
SUMMARY = (
    ...
    # **The dropout** (issue #55). `music` is frames the siren was heard
    # wailing on and `nomus` is frames nothing else wanted and the building
    # had already spent -- the second is the one the design's claim rests on,
    # and the two are meant to be read as a ratio rather than singly. Since
    # the siren (issue #67) `nomus` should be zero in any room the sample has
    # seen: the wail holds a pitch to eighteen Clegs and a click to the
    # ceiling. `music` is a third of the frames at most, because the siren
    # rests for two thirds of every cycle, and that is not in either column.
