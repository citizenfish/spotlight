...
| `--gallery DIR` | Writes the sheets a look-and-feel review needs and prints the paths: the title screen (both halves of its flash), the ending screen, a labelled sprite sheet with both walk frames of every figure, each room fully lit and as it looks a few seconds into a run — the same instant twice, with everybody on walk frame A and then on frame B, so the walk can be judged where a figure is actually seen — and the frames a bot cannot produce on its own — a spotlight burning on the floor, a moment's flash, the mains surge, and **a body in a played room beside somebody standing up**. Runs no seeds and writes no report. |
